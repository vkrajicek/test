...
class Book_repository:
    ...
    # get all books from bok table accoring to user_id
    ...
    # get one row of books from book table according to id
    ...
    def add_new_book(self, book):
        ...
        for i in book.chapters:
            ...
            insert_query_new_chapter = "INSERT INTO projekt.chapter (id, chapter_name, edited_date) VALUES (%s, %s, now())"
            record_to_insert_new_chapter = (int(chapter_id) + 1, i.chapter_name)
            self.cursor.execute(insert_query_new_chapter, record_to_insert_new_chapter)
            self.connection.commit()
        ...
        for i in chapter_ids_list:
            ...
            insert_query_new_book_and_chapters = "INSERT INTO projekt.book_chapter (id, book_id, chapter_id, edited_date) VALUES (%s, %s, %s, now())"
            record_to_insert_new_book_and_chapters = (book_chaper_id, int(book.id), int(i))
            self.cursor.execute(insert_query_new_book_and_chapters, record_to_insert_new_book_and_chapters)
            self.connection.commit()
        return book
    ...
    def book_delete(self, book_id):
        ...
        book_chapter_delete_query = ("DELETE from projekt.book_chapter where book_id=%d" % book_id)
        self.cursor.execute(book_chapter_delete_query)
        chapter_delete_query = ("DELETE from projekt.chapter where id = %d;" % chapters_ids_list[0])
        self.cursor.execute(chapter_delete_query)
        book_delete_query = "DELETE FROM projekt.book WHERE id = %s" % book_id
        self.cursor.execute(book_delete_query)
        self.connection.commit()
